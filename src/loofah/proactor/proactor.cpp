...
bool Proactor::initialize() noexcept
{
    ...
    if (nullptr == _iocp)
    {
        LOOFAH_LOG_ERR(CreateIoCompletionPort);
        return false;
    }
#elif NUT_PLATFORM_OS_MACOS
    // Create kqueue
...
void Proactor::register_handler(ProactHandler *handler) noexcept
{
    ...
    if (nullptr == handler->_attached_iocp)
    {
        ...
        if (nullptr == rs)
        {
            ...
            LOOFAH_LOG_ERR_CODE(CreateIoCompletionPort, errcode);
...
#   else
    ::CancelIo((HANDLE) fd); // 取消当前线程注册的尚未完成的异步操作。由于都是在轮询线程中发起的异步操作，故都能取消掉
#   endif
    handler->cancel_requests();
#elif NUT_PLATFORM_OS_MACOS
    struct kevent ev[2];
    int n = 0;
...
void Proactor::launch_accept(ProactHandler *handler) noexcept
{
    ...
    if (FALSE == rs)
    {
        ...
        {
            LOOFAH_LOG_ERR_FD(AcceptEx, listening_socket);
            if (0 != ::closesocket(accept_socket))
                LOOFAH_LOG_ERR_FD(closesocket, accept_socket);
            IORequest::delete_request(io_request);
            handler->handle_io_error(from_errno(errcode));
            return;
        }
    }
    handler->_read_queue.push_back(io_request);
...
void Proactor::launch_connect(ProactHandler *handler, const InetAddr& address) noexcept
{
    ...
    if (FALSE == rs)
    {
        ...
        if (ERROR_IO_PENDING != errcode)
        {
            LOOFAH_LOG_ERR_FD(ConnectEx, fd);
            IORequest::delete_request(io_request);
            handler->handle_io_error(from_errno(errcode));
            return;
        }
    }
    handler->_write_queue.push_back(io_request);
}
#else
...
int Proactor::poll(int timeout_ms) noexcept
{
    ...
    const BOOL rs = ::GetQueuedCompletionStatus(
        _iocp, &bytes_transfered, &key, &io_overlapped, timeout);
    ...
    else if (FALSE == rs && ERROR_SUCCESS != errcode) // case 4: 连接错误
    {
        ...
        else
        {
            ...
            if (0 != (io_request->event_type & ProactHandler::ACCEPT_READ_MASK))
            {
                ...
                handler->_read_queue.pop_front();
            }
            else
            {
                assert(io_request == handler->_write_queue.front());
                handler->_write_queue.pop_front();
            }
            IORequest::delete_request(io_request);
    ...
    else // case 3, 6: 连接关闭 / 正常返回
    {
        ...
        else
        {
            ...
            case ProactHandler::CONNECT_MASK:
            {
                ...
                if (0 != opt_rs)
                {
                    LOOFAH_LOG_ERR_FD(setsockopt, fd);
...
void Proactor::wakeup_poll_wait() noexcept
{
    ...
    if (!::PostQueuedCompletionStatus(_iocp, 0, 0, nullptr))
        LOOFAH_LOG_ERR(PostQueuedCompletionStatus);
#elif NUT_PLATFORM_OS_MACOS
    struct kevent ev;
